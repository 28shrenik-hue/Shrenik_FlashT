...
powershell -NoProfile -Command "$target='release\FlashTile-v1.0.0-rc16-win64.zip'; if (Test-Path $target) { Remove-Item $target }; Compress-Archive -Path 'dist\FlashTile\*' -DestinationPath $target"
if errorlevel 1 exit /b 1

echo.
echo Windows package created:
echo release\FlashTile-v1.0.0-rc16-win64.zip
